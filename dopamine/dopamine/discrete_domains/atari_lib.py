...
@gin.configurable
def create_atari_environment(game_name=None, sticky_actions=True, labels=[]):
  ...
  if name == 'james_bond':
      name = 'jamesbond'
  # safety labelling
  labellers = [get_property(env, name, label) for label in labels]
  env = LabellingEnv(env, labellers)
...
class AtariPreprocessing(object):
  ...
  def step(self, action):
    ...
    for time_step in range(self.frame_skip):
      ...
      accumulated_cost += 1.0 * info['label']
    ...
    return observation, accumulated_reward, accumulated_cost, is_terminal, info
