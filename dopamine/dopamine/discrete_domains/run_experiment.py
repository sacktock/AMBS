...
class Runner(object):
  ...
  def _initialize_checkpointer_and_maybe_resume(self, checkpoint_file_prefix):
    ...
    # Check if start iteration is > 0. This means there is cummulative statistics to load
    print("WARNING: Loading cumulative metrics is not implemented")
    '''if self._start_iteration > 0:
      current_iteration = int(self._start_iteration-1)
      objects = []
      path = os.path.join(self._base_dir, 'metrics/pickle')
      with (open(path+"/pickle_{}.pkl".format(max(0, current_iteration-1)), "rb")) as openfile:
        while True:
          try:
            objects.append(pickle.load(openfile))
          except EOFError:
            break
      for iteration in range(1, max(current_iteration+1, 2)):
        self._total_violations += int(objects[-1]['iteration_{}'.format(iteration)]['Train/SumCosts'][-1])'''
  ...
  def _run_one_step(self, action):
    ...
    Returns:
      The observation, reward, and is_terminal values returned from the
        environment.
    """
    observation, reward, cost, is_terminal, _ = self._environment.step(action)
    return observation, reward, cost, is_terminal
  ...
  def _run_one_episode(self):
    ...
    while True:
      observation, reward, cost, is_terminal = self._run_one_step(action)
    ...
    return step_number, total_reward, total_cost

  def _save_tensorboard_episodes(self, iteration, statistics):
    if self._summary_writer is None:
      print("self._summary_writer is None")
      return

    if 'train_episode_lengths' not in statistics.keys():
      print("'train_episode_lengths' not in statistics.keys()")
      return
    # default frame_skip is 4 change if necessary
    FRAME_SKIP = 4
    step = iteration * self._training_steps * FRAME_SKIP
    assert len(statistics['train_episode_lengths']) == len(statistics['train_episode_returns']) \
      == len(statistics['train_episode_costs']) == len(statistics['train_episode_violations'])
    
    for idx in range(0, len(statistics['train_episode_lengths'])):
        episode_length = statistics['train_episode_lengths'][idx]
        episode_return = statistics['train_episode_returns'][idx]
        episode_cost = statistics['train_episode_costs'][idx]
        episode_violations = statistics['train_episode_violations'][idx]
        self._total_violations += episode_violations
        step += episode_length * FRAME_SKIP
        if self._sess is None:
          with self._summary_writer.as_default():
            tf.summary.scalar('episode/train_length', episode_length, step=step)
            tf.summary.scalar('episode/train_return', episode_return, step=step)
            tf.summary.scalar('episode/train_cost', episode_cost, step=step)
            tf.summary.scalar('episode/train_violations', episode_violations, step=step)
            tf.summary.scalar('episode/train_violation_rate', float(episode_violations/episode_length), step=step)
            tf.summary.scalar('episode/train_total_violations', self._total_violations, step=step)
          self._summary_writer.flush()
        else:
          summary = tf.compat.v1.Summary(value=[
            tf.compat.v1.Summary.Value(tag='episode/train_length', simple_value=episode_length),
            tf.compat.v1.Summary.Value(tag='episode/train_return', simple_value=episode_return),
            tf.compat.v1.Summary.Value(tag='episode/train_cost', simple_value=episode_cost),
            tf.compat.v1.Summary.Value(tag='episode/train_violations', simple_value=episode_violations),
            tf.compat.v1.Summary.Value(tag='episode/train_violation_rate', simple_value=float(episode_violations/episode_length)),
            tf.compat.v1.Summary.Value(tag='episode/train_total_violations', simple_value=self._total_violations),
          ])
  ...
  def _run_one_phase(self, min_steps, statistics, run_mode_str):
    ...
    while step_count < min_steps:
      episode_length, episode_return, episode_cost = self._run_one_episode()
      statistics.append({
          '{}_episode_lengths'.format(run_mode_str): episode_length,
          '{}_episode_returns'.format(run_mode_str): episode_return,
          '{}_episode_costs'.format(run_mode_str): episode_cost,
          '{}_episode_violations'.format(run_mode_str): episode_cost,
      })
      step_count += episode_length
      sum_returns += episode_return
      sum_costs += episode_cost
      num_episodes += 1
      if self._fine_grained_print_to_console:
        ...
        sys.stdout.write('Steps executed: {} '.format(step_count) +
                         ...
                         'Cost: {} '.format(episode_cost) +
                         'Violations: {}\r'.format(episode_cost))
        sys.stdout.flush()
    return step_count, sum_returns, sum_costs, num_episodes
  ...
  def _run_train_phase(self, statistics):
    ...
    number_steps, sum_returns, sum_costs, num_episodes = self._run_one_phase(
        self._training_steps, statistics, 'train')
    average_return = sum_returns / num_episodes if num_episodes > 0 else 0.0
    average_cost = sum_costs / num_episodes if num_episodes > 0 else 0.0
    statistics.append({'train_average_return': average_return})
    statistics.append({'train_average_cost': average_cost})
    statistics.append({'train_sum_cost': sum_costs})
    ...
    logging.info('Average undiscounted return per training episode: %.2f',
                 average_return)
    logging.info('Average undiscounted cost per training episode: %.2f',
                 average_cost)
    logging.info('Summation of the undiscounted costs: %.2f', sum_costs)
    logging.info('Average training steps per second: %.2f',
                 average_steps_per_second)
    return num_episodes, average_return, average_cost, sum_costs, average_steps_per_second
  ...
  def _run_eval_phase(self, statistics):
    ...
    _, sum_returns, sum_costs, num_episodes = self._run_one_phase(
    ...
    statistics.append({'eval_average_cost': average_cost})
    statistics.append({'eval_sum_cost': sum_costs})
    return num_episodes, average_return, average_cost, sum_costs
  ...
  def _run_one_iteration(self, iteration):
    ...
    num_episodes_train, average_reward_train, average_cost_train, sum_cost_train, average_steps_per_second = (
        self._run_train_phase(statistics))
    num_episodes_eval, average_reward_eval, average_cost_eval, sum_cost_eval = self._run_eval_phase(
        statistics)
    ...
    if self._has_collector_dispatcher:
      self._collector_dispatcher.write([
          ...
          statistics_instance.StatisticsInstance('Train/AverageReturns',
                                                 ...
                                                 iteration),
          statistics_instance.StatisticsInstance('Train/AverageCosts',
                                                 average_cost_train,
                                                 iteration),
          statistics_instance.StatisticsInstance('Train/SumCosts',
                                                 sum_cost_train,
                                                 iteration),
          ...
          statistics_instance.StatisticsInstance('Eval/AverageReturns',
                                                 average_reward_eval,
                                                 iteration),
          statistics_instance.StatisticsInstance('Eval/AverageCosts',
                                                 average_cost_eval,
                                                 iteration),
          statistics_instance.StatisticsInstance('Eval/SumCosts',
                                                 sum_cost_eval,
                                                 iteration)
      ])
    if self._summary_writer is not None:
      self._save_tensorboard_summaries(iteration, num_episodes_train,
                                       average_reward_train, average_cost_train,
                                       num_episodes_eval, average_reward_eval,
                                       average_cost_eval, average_steps_per_second)
    ...
    if self._sess is None:
      with self._summary_writer.as_default():
        ...
        tf.summary.scalar('Train/AverageReturns', average_reward_train,
                          step=iteration)
        tf.summary.scalar('Train/AverageCosts', average_cost_train,
                          step=iteration)
        ...
        tf.summary.scalar('Eval/AverageReturns', average_reward_eval,
                          step=iteration)
        tf.summary.scalar('Eval/AverageCosts', average_cost_eval,
                          step=iteration)
      self._summary_writer.flush()
    else:
      summary = tf.compat.v1.Summary(value=[
          ...
          tf.compat.v1.Summary.Value(
              tag='Train/AverageCosts', simple_value=average_cost_train),
          tf.compat.v1.Summary.Value(
          ...
          tf.compat.v1.Summary.Value(
              ...
              tf.compat.v1.Summary.Value(
              tag='Eval/AverageCosts', simple_value=average_cost_eval)
      ])
  ...
  def run_experiment(self):
    ...
    for iteration in range(self._start_iteration, self._num_iterations):
      ...
      if self._summary_writer is not None:
        self._save_tensorboard_episodes(iteration, statistics)
      if self._use_legacy_logger:
...
class TrainRunner(Runner):
  ...
  def _save_tensorboard_summaries(self, iteration, num_episodes, average_reward, 
                                  average_cost, average_steps_per_second):
    """Save statistics as tensorboard summaries."""
    if self._summary_writer is None:
      return
    ...
    if self._sess is None:
      with self._summary_writer.as_default():
        ...
        tf.summary.scalar('Train/AverageReturns', average_reward,
                          step=iteration)
        tf.summary.scalar('Train/AverageCosts', average_cost,
                          step=iteration)
        tf.summary.scalar('Train/AverageStepsPerSecond',
                          average_steps_per_second, step=iteration)
      self._summary_writer.flush()
      ...
      summary = tf.compat.v1.Summary(value=[
          ...
          tf.compat.v1.Summary.Value(
              tag='Train/AverageCosts', simple_value=average_Cost),
          tf.compat.v1.Summary.Value(
              tag='Train/AverageStepsPerSecond',
              simple_value=average_steps_per_second),
      ])
