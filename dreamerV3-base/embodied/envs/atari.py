...
class Atari(embodied.Env):
  ...
  def __init__(
      ...
      sticky=True, actions='all', length=108000, resize='opencv', seed=None, labels=[], cost_val=1.0):
    ...
    # safety labelling
    labellers = [get_property(self._env, name, label) for label in labels]
  ...
  def step(self, action):
    if action['reset'] or self._done:
      ...
      self._done = False
      self._step = 0
      return self._obs(0.0, 0.0, is_first=True)
    total = 0.0
    total_cost = 0.0
    dead = False
    for repeat in range(self._repeat):
      ...
      self._step += 1
      total += reward
      total_cost += self._cost_val * info['label']
  ...
  def _obs(self, reward, cost, is_first=False, is_last=False, is_terminal=False):
