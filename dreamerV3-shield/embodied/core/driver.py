...
class Driver:
  ...
  def __init__(self, env, cumul_metrics, **kwargs):
    assert len(env) > 0
    self._env = env
    self._cumul_metrics = cumul_metrics
    self._kwargs = kwargs
    self._on_steps = []
    self._on_episodes = []
    self.reset()
  ...
  def _step(self, policy, step, episode):
    ...
    if self._mets:
      obs.update(self._mets)
    else:
      shape = obs['is_last'].shape
      obs.update({
        'override' : np.full(shape, False),
        'prob' : np.zeros(shape),
      })
    obs = {k: convert(v) for k, v in obs.items()}
    assert all(len(x) == len(self._env) for x in obs.values()), obs
    acts, self._state, mets = policy(obs, self._state, **self._kwargs)
    mets = {k: convert(v) for k, v in mets.items()}
    ...
    if obs['is_last'].any():
      for i, done in enumerate(obs['is_last']):
        if done:
          ...
          [fn(ep.copy(), i, self._cumul_metrics, **self._kwargs) for fn in self._on_episodes]
          episode += 1
    return step, episode
